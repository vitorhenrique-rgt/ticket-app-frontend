...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchTicketStats = async () => {
      ...
      try {
        const response = await axios.get(`${apiUrl}/api/tickets/?startDate=${startDate}&endDate=${endDate}&limit=30000`);
        const data = response.data.tickets;
        if (filter === 'day') {
          // Processar tickets por hora
          const hours = Array.from({ length: 24 }, (_, i) => i); // [0, 1, 2, ..., 23]
          const ticketsByHour = hours.map(hour => {
            return data.filter(ticket => new Date(ticket.createdAt).getHours() === hour).length;
          });
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-900">
      <div className="max-w-4xl w-full bg-gray-800 p-8 rounded-lg shadow-md">
        <h2 className="text-2xl text-white mb-6 text-center">Dashboard</h2>
        <div className="flex justify-center gap-4 mb-8">
          <button onClick={() => setFilter('day')} className={`btn ${filter === 'day' ? 'bg-indigo-500' : 'bg-gray-700'}`}>Dia</button>
          <button onClick={() => setFilter('week')} className={`btn ${filter === 'week' ? 'bg-indigo-500' : 'bg-gray-700'}`}>Semana</button>
          <button onClick={() => setFilter('month')} className={`btn ${filter === 'month' ? 'bg-indigo-500' : 'bg-gray-700'}`}>Mês</button>
        </div>
        <div className="mb-8">
          <h3 className="text-lg text-white mb-4">Quantidade de Chamados</h3>
          <canvas ref={ticketChartRef} id="ticketChart"></canvas>
        </div>
        <div className="mb-8">
          <h3 className="text-lg text-white mb-4">Tags Mais Usadas</h3>
          <canvas ref={tagChartRef} id="tagChart"></canvas>
        </div>
        <div className="flex justify-between mt-8">
          <button onClick={() => navigate('/admin/users')} className="btn bg-green-500">Usuários</button>
          <button onClick={() => navigate('/admin/companies')} className="btn bg-blue-500">Empresas</button>
          <button onClick={() => navigate('/admin/tags')} className="btn bg-purple-500">Tags</button>
        </div>
      </div>
    </div>
  );
